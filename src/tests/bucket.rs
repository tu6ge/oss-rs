...
async fn test_get_bucket_info() {
    ...
    impl Middleware for MyMiddleware {
        async fn handle(&self, request: Request) -> OssResult<Response> {
            ...
            assert_eq!(
                *request.url(),
                Url::parse("https://foo4.oss-cn-shanghai.aliyuncs.com/?bucketInfo").unwrap()
            );
            assert_eq!(
                request.headers().get("canonicalizedresource"),
...
fn test_get_blocking_bucket_info() {
    ...
    assert_eq!(
        format!("{:?}", res),
        r#"Ok(Bucket { base: BucketBase { endpoint: CnShanghai, name: BucketName("barname") }, creation_date: 2016-11-05T13:10:10Z, intranet_endpoint: "oss-cn-shanghai-internal.aliyuncs.com", location: "oss-cn-shanghai", storage_class: "Standard" })"#
    );
}
